/* ==========================================================================
   Canela - Tourist Spot Data
   Consumed by ../assets/app.js
   ========================================================================== */

window.CITY_NAME = 'Canela';

window.CITY_DATA = [
  {
    id: 'ice_bar_mundo_gelado',
    name: 'Ice Bar Mundo Gelado',
    category: 'Entretenimento',
    city: 'Canela',
    address: 'Rodovia RS-466, 3915 - Caracol',
    hours: 'Segunda a sábado: 19h às 23h | Domingo: fechado',
    price: 'R$ 75,00. Consulte o site para ver valores no período da sua viagem.',
    rating: '4.4 ★ (2.4k avaliações)',
    popularRank: 1,
    route: 1,
    image: 'imagens/ice_bar.jpg',
    summary: 'Bar de gelo onde pode-se experimentar drinks em copos de gelo em um ambiente com temperatura que pode chegar a -23 °C.',
    description: 'O Ice Bar Mundo Gelado é uma das atrações mais diferentes de Canela e faz parte do complexo Mundo Gelado. O espaço reproduz uma caverna congelada com esculturas de gelo em formatos como sofás, mesas, camas e trenós. A experiência inclui dois drinks servidos em copos de gelo e agasalhos térmicos com capuz e luvas. O complexo também oferece a Casa do Capitão, espaço temático militar, além de loja, cafeteria, sorveteria e museu aberto à noite.',
    tip: 'Vista calça comprida e calçados fechados e aproveite para fotografar as esculturas de gelo. Reserve um tempo para conhecer também a Casa do Capitão e o museu do complexo.'
  },
  {
    id: 'praca_joao_correa',
    name: 'Praça João Corrêa',
    category: 'Paisagem',
    city: 'Canela',
    address: 'Canela - Centro',
    hours: 'Acesso 24h',
    price: 'Gratuito',
    rating: '4.1 ★ (112 avaliações)',
    popularRank: 2,
    route: 2,
    image: 'imagens/praca_joao_correa.png',
    summary: 'Principal praça do centro de Canela, cercada por atrações culturais, comércio, artesanato e eventos.',
    description: 'A Praça João Corrêa é um dos principais espaços públicos de Canela e recebeu o nome em homenagem ao fundador da cidade. A praça funciona como ponto de encontro e palco para eventos culturais, apresentações musicais e festividades ao longo do ano.',
    tip: 'Combine a visita com a Estação Campos de Canella, a Catedral de Pedra e o comércio do centro.'
  },
  {
    id: 'estacao_campos_canella',
    name: 'Antiga Estação Ferroviária Canela',
    category: 'Atração',
    city: 'Canela',
    address: 'Largo Benito Urbani - Centro',
    hours: 'Diariamente: 10h às 22h',
    price: 'Gratuito',
    rating: '4.7 ★ (2k+ avaliações)',
    popularRank: 3,
    route: 3,
    image: 'imagens/estacao_campos_canella.png',
    summary: 'Antiga estação ferroviária revitalizada que atualmente abriga o complexo, com gastronomia, comércio e entretenimento.',
    description: 'A antiga estação ferroviária de Canela foi inaugurada em 1924 e teve papel importante no desenvolvimento turístico e econômico da cidade. Atualmente, a Estação Campos de Canella preserva elementos da história ferroviária e reúne restaurantes, lojas, entretenimento e uma locomotiva restaurada.',
    tip: 'Observe a locomotiva e os elementos históricos preservados antes de conhecer os restaurantes e lojas do complexo.'
  },
  {
    id: 'catedral_de_pedra',
    name: 'Catedral de Pedra',
    category: 'Atração',
    city: 'Canela',
    address: 'Praça Matriz, 69 - Centro',
    hours: 'Diariamente: 08h às 19h',
    price: 'Gratuito',
    rating: '4.9 ★ (2.6k avaliações)',
    popularRank: 4,
    route: 4,
    image: 'imagens/catedral_de_pedra.png',
    summary: 'Imponente igreja em estilo gótico inglês, considerada um dos maiores símbolos arquitetônicos de Canela.',
    description: 'A Catedral de Pedra, oficialmente Paróquia Nossa Senhora de Lourdes, é um dos principais cartões-postais de Canela. Construída em pedra e inspirada na arquitetura gótica, destaca-se pela torre, vitrais e iluminação noturna que valoriza sua fachada.',
    tip: 'Visite durante o dia para observar os detalhes arquitetônicos e retorne à noite para apreciar a iluminação da catedral.'
  },
  {
    id: 'sonho_natal_canela_2026',
    name: 'Sonho de Natal de Canela 2026',
    category: 'Natal',
    city: 'Canela',
    address: 'Diversos locais de Canela - Centro',
    hours: 'Temporada de Natal 2026 - programação a confirmar',
    price: 'Atrações gratuitas e espetáculos conforme programação',
    rating: '4.3 ★ (44 avaliações)',
    popularRank: 7,
    route: 7,
    image: 'imagens/natal_canela_2026.png',
    summary: 'Tradicional evento natalino de Canela que transforma a cidade com iluminação, música e espetáculos.',
    description: 'O Sonho de Natal é um dos principais eventos turísticos de Canela. A edição de 2026 corresponde à 39ª edição do evento e promete atrações culturais, decoração especial, iluminação natalina, música, teatro e atividades para toda a família.',
    tip: 'A programação oficial de 2026 ainda deve ser consultada antes da viagem, pois horários e espetáculos podem ser divulgados posteriormente.'
  },
  {
    id: 'tri_hotel_canela',
    name: 'Tri Hotel Canela',
    category: 'Hotel',
    address: 'R. José Pedro Piva, 220 - Vila Suzana',
    hours: 'Check-in: a partir das 14h | Check-out: até 12h | Recepção: 24 horas',
    price: 'Diárias: consulte os valores para a data da hospedagem',
    rating: '4.2 ★ (1.258 avaliações)',
    popularRank: 2,
    route: 10,
    image: 'imagens/tri_hotel_canela.jpg',
    summary: 'Hotel tradicional de Canela, com piscina coberta, restaurante, localizado próximo às principais atrações da Serra Gaúcha.',
    description: 'O Tri Hotel Canela está localizado no bairro Vila Suzana e oferece uma estrutura completa para quem visita Canela e Gramado. O hotel conta com recepção 24 horas, Wi-Fi gratuito, restaurante, bar, piscina coberta, academia, espaço kids, estacionamento, sala para eventos e serviço de quarto. O café da manhã é servido diariamente e o restaurante Tri Gastronomia funciona das 11h30 às 23h. A hospedagem fica a poucos quilômetros de atrações como a Catedral de Pedra e o Mundo a Vapor.',
    tip: 'O hotel é uma boa base para explorar Canela e Gramado de carro. Se estiver viajando em família, vale verificar a disponibilidade da piscina coberta e do espaço kids.'
  }
];
